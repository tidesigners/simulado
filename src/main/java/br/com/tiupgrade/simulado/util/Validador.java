...
public final class Validador {
     ...
     */
    public static boolean validaExtensao(UploadedFile event) {
        return event.getFileName().endsWith("xlsx") || event.getFileName().endsWith("xls") ;
    }
     ...
     * @return
     */
    public static String converteTamanho(FileUploadEvent event) {

        final String[] unidades = new String[]{"B", "kB", "MB", "GB", "TB"};
        int grupos = (int) (Math.log10(event.getFile().getSize()) / Math.log10(1024));
        String tamanho = new DecimalFormat("#,##0.#").format(event.getFile().getSize() / Math.pow(1024, grupos)) + " " + unidades[grupos];
